...
function solution(N) {
    ...
    /* 
        "10000".split("1") => ["", "0000"] => if (length===2) X
        "100001".split("1") => ["", "0000", ""] => arr[1].length
        "100001000".split("1") => ["", "0000", "000"] => arr[1].length
        "100001000".split("1") => ["", "0000", "000", ""] => arr[1 || 2].length
         => index[0], index[last] => X, 
    */

    function getMaxLenght(arr) {
        let newArr = arr.slice(1)
        newArr.pop()

        newArr = newArr.map((str)=> str.length);
        return Math.max(...newArr)

    }

    return arr.length > 2 ? getMaxLenght(arr) : 0;
}